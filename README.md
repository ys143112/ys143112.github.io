...
- 이미지나 코드 블록만 있는 글도 발행할 수 있습니다. 이때 소개문을 직접 적으면 목록과 공유 설명이 더 알기 쉬워집니다.
- 본문 HTML은 허용된 요소만 남기며 스크립트, 이벤트 속성, 위험한 링크는 제거됩니다. HTML 코드 예제는 코드 블록으로 작성하세요.
...
**공개 GitHub 저장소에서는 초안 파일, 업로드한 이미지, 과거 수정 이력도 공개됩니다.** 업로드 보관함의 파일은 글 공개 여부와 관계없이 사이트에도 복사됩니다. 사이트의 숨김 기능은 원고의 비밀 보관 기능이 아닙니다. 민감한 초안은 로컬 등 비공개 위치에 보관하세요.
...
홈, 기록 목록, 카테고리, 프로젝트 목록, 글 상세, 소개, 글쓰기 안내, 404 화면을 포함합니다. 검색은 현재 목록의 제목·본문·코드·태그를 브라우저에서 필터링합니다. 여러 검색어는 모두 포함된 글을 찾으며, 검색어는 주소에 보존되어 새로고침과 카테고리 이동 후에도 유지됩니다. 모바일 글 상세의 접이식 목차로 원하는 항목에 이동할 수 있습니다.

블로그 이름을 바꾸면 상단과 하단 로고에도 반영됩니다. 글에는 작성자·게시일·수정일을 담은 구조화 데이터를 제공합니다.
...
사진은 회전 정보를 반영한 실제 출력 크기로 최적화합니다. 삭제한 원본의 이전 최적화 파일은 다음 빌드에서 정리하며, 원본 보관함은 자동 삭제하지 않습니다.

전문가 검토 결과, 변경 이유, 검증 범위와 남은 제약은 [REVIEW.md](REVIEW.md)에 정리했습니다.
